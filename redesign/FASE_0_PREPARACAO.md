...
**Fase:** 0 de 8  
**Modelo recomendado:** Manual ou Haiku  
...
### O que é o BO Inteligente?
Sistema que ajuda policiais militares a redigir Boletins de Ocorrência (BOs) através de um chat com IA. O usuário responde perguntas e o sistema gera o texto técnico-jurídico automaticamente.

### Por que este redesign?
O sistema cresceu de 6 para 53 perguntas distribuídas em 8 seções. A interface atual (sidebar com lista de perguntas) não escala bem. O novo design usa:
- **Barra de progresso horizontal** (estilo Duolingo)
- **Seções independentes** (uma por vez, sem scroll infinito)
- **Componentes de input variados** (texto livre, escolha única, múltipla escolha)

### Documentos de referência
Estes arquivos estão na pasta `/redesign/` do projeto:
- `PROPOSTA_REDESIGN_UX_BO_INTELIGENTE.md` - Design e wireframes
- `PLANO_IMPLEMENTACAO_REDESIGN_UX.md` - Plano técnico com todas as fases
...
1. Criar branch Git para o redesign
2. Fazer backup do código atual
3. Criar estrutura de pastas
4. Criar arquivo de configuração das seções (`sections.js`)

---

## ✅ Tarefas

### Tarefa 0.1: Criar branch de desenvolvimento

**Objetivo:** Isolar as mudanças do redesign em uma branch separada.
...
```bash
git branch
# Deve mostrar: * feature/ux-redesign-v1
```

---

### Tarefa 0.2: Fazer backup do index.html atual

**Objetivo:** Preservar a versão atual caso precise de rollback.

**Comandos:**
```bash
cd /caminho/para/bo-assistant/docs
cp index.html index.html.backup-v0.4.1
```
...
### Tarefa 0.3: Criar pasta para documentação do redesign

**Objetivo:** Organizar os documentos de design e implementação.
...
**Ação manual:** Copiar os arquivos de referência para esta pasta:
- `PROPOSTA_REDESIGN_UX_BO_INTELIGENTE.md`
- `PLANO_IMPLEMENTACAO_REDESIGN_UX.md`
- Documentos de cada fase (como este)

---

### Tarefa 0.4: Criar estrutura de pastas para JavaScript

**Objetivo:** Preparar estrutura para futura refatoração (Fase 8).
...
docs/
├── index.html
├── index.html.backup-v0.4.1
├── js/
│   └── data/
│       └── (sections.js será criado na próxima tarefa)
└── logs.html
```

---

### Tarefa 0.5: Criar arquivo de configuração das seções

**Objetivo:** Definir a estrutura de dados de todas as 8 seções e suas perguntas.

**Arquivo:** `docs/js/data/sections.js`
...
**Verificação:**
Abrir o arquivo `docs/js/data/sections.js` e confirmar que:
- Tem 8 seções definidas
- Cada seção tem `id`, `name`, `emoji`, `questions`
- Perguntas têm `id`, `text`, `inputType`

---

### Tarefa 0.6: Testar carregamento do sections.js

**Objetivo:** Verificar se o arquivo JavaScript está correto.
...
5. Abrir o Console do desenvolvedor (F12)
6. Digitar:
```javascript
// Carregar o script manualmente para teste
const script = document.createElement('script');
script.src = 'js/data/sections.js';
document.head.appendChild(script);
```

7. Aguardar 1 segundo e digitar:
```javascript
console.log(SECTIONS_DATA);
console.log('Total de seções:', SECTIONS_DATA.length);
console.log('Total de perguntas:', countTotalQuestions());
```

**Resultado esperado:**
```
[sections.js] Carregado: 8 seções, ~53 perguntas
Array(8) [...]
Total de seções: 8
...
### Tarefa 0.7: Commit inicial

**Objetivo:** Salvar o progresso no Git.
...
git status
# Deve mostrar:
# - docs/index.html.backup-v0.4.1
# - docs/js/data/sections.js
# - redesign/ (se já tiver os docs)

git commit -m "chore: preparação para redesign UX v1

- Criar branch feature/ux-redesign-v1
- Backup do index.html atual
- Criar estrutura de pastas js/data
- Criar sections.js com 8 seções e ~53 perguntas
- Adicionar docs de redesign em /redesign"
...
- [ ] Branch `feature/ux-redesign-v1` criada e ativa
- [ ] Backup `docs/index.html.backup-v0.4.1` existe
- [ ] Pasta `docs/js/data/` criada
- [ ] Arquivo `docs/js/data/sections.js` criado com 8 seções
- [ ] Teste no console mostra 8 seções e ~53 perguntas
- [ ] Commit feito e pushado
- [ ] Pasta `redesign/` com documentos de referência
...
- Modelo: 🟢 Haiku
- Arquivo: `FASE_1_BARRA_PROGRESSO.md`
- Objetivo: Criar componente visual da barra de progresso horizontal

---

## 📚 Referências

- `redesign/PROPOSTA_REDESIGN_UX_BO_INTELIGENTE.md` - Design completo
- `redesign/PLANO_IMPLEMENTACAO_REDESIGN_UX.md` - Plano de todas as fases
